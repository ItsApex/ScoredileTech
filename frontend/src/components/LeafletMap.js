...
const tempPloyline = [[72.816449,19.205002],[72.816789,19.204936],[72.817027,19.205185]]
...
function LeafletMap({ lat, lng, dialogtxt }) {
  ...
  useEffect(() => {
    ...
    axios
     ...
     axios
    .get("https://sachet.ndma.gov.in/cap_public_website/FetchIMDNowcastAlerts")
    .then((response) => {
     ...
     .then((res)=>{
      ...
      const bufferedPolygons = res.data.map((disaster) => {
        ...
        const circleCenter = point([parseFloat(disaster.longitude), parseFloat(disaster.latitude)]);
        const bufferedPolygon = buffer(circleCenter, radiusInMeters, { units: 'meters' });
        return bufferedPolygon.geometry.coordinates[0].map((coord) => [coord[1], coord[0]]);
      });
    ...
    if (dialogtxt === "R") {
      const { lat, lng } = e.latlng;
        ...
        try {
          ...
          const avoidPolygonsParam = `&avoid_polygons=${JSON.stringify(avoidPolygons)}`;
          ...
          const swappedRouteCoords = routeCoords.map((coord) => [coord[1], coord[0]]);
